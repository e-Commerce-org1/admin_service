...
export class UserService {
  ...
  async getAllUsers(params: GetAllUsersRequest): Promise<GetAllUsersResponse> {
    ...
    this.logger.log(`Starting to fetch all users with params: ${JSON.stringify(params)}`);
  ...
  async getUserById(params: GetUserByIdRequest): Promise<GetUserByIdResponse> {
    ...
    try {
      ...
      this.logger.error(`Failed to fetch user ${params.userId}: ${error.message}`);
      throw new Error(`Failed to fetch user ${params.userId}: ${error.message}`);
    }
  }

  async updateUserStatus(params: UpdateUserStatusRequest): Promise<UpdateUserStatusResponse> {
    ...
    try {
      ...
      this.logger.error(`Failed to update status for user ${params.userId}: ${error.message}`);
      throw new Error(`Failed to update status for user ${params.userId}: ${error.message}`);
    }
  }
  ...
  async searchUsers(params: SearchUsersRequest): Promise<SearchUsersResponse> {
    ...
    try {
      ...
      this.logger.log(`Search completed with ${result.users?.length || 0} results`);
      return result;
  ...
  async unblockUser(params: UnblockUserRequest): Promise<UnblockUserResponse> {
    ...
    try {
      ...
      this.logger.error(`Failed to unblock user ${params.userId}: ${error.message}`, error.stack);
      throw new Error(`Failed to unblock user ${params.userId}: ${error.message}`);
    }
  }
}